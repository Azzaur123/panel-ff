...
do
    local Window = Fluent:CreateWindow({
        Title = string.format("%s <b><i>%s</i></b>", string.format(MonthlyLabels[os.date("*t").month], "ของดีบอกต่อ"), #Status > 0 and Status or "🔥FREE🔥"),
        SubTitle = "By @xeno_executor",
    ...
    if IsComputer then
        local OnePressAimingModeToggle = AimbotSection:AddToggle("OnePressAimingMode", { Title = "OnePressAimingMode", Description = "Uses the One-Press Mode instead of the Holding Mode", Default = Configuration.OnePressAimingMode })
    ...
    if UISettings.ShowWarnings then
        ...
        elseif ShowWarning then
            Window:Dialog({
                Title = "XENO EZ | AIMBOT",
                Content = string.format("ของฟรีมีในโลก Aim Bot %s  🔥FREE🔥!", string.format(MonthlyLabels[os.date("*t").month], "Open Aimbot")),
                Buttons = {
                    {
                        Title = "Confirm"
                    }
